...
export const ActionStepSchema = zod.object({
  welcome: zod.string().min(1, { message: 'Please enter a welcome message' }),
});
...
export function StartForm({ step }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const updatedSteps = steps.map((s: ActionStep) =>
        s.id === step.id ? { ...s, metadata: { welcome: data.welcome } } : s
      );
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      <Stack spacing={4}>
        <Stack spacing={1}>
          <Typography variant="body2">First Message</Typography>
          <Field.Text name="welcome" multiline rows={4} />
        </Stack>
